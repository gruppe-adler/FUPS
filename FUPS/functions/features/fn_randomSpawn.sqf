...
switch (_type) do {
	case (0): { // infantry
		...
		_spawnpos = if (_pos isEqualTo []) then {getPosATL leader _group } else {_pos};
	...
	case (1): { // land vehicle
		...
		{
			private "_rel";
			_rel = (_vehicles select 0) worldToModel (getPosATL _x);
			_rel set [2,0];
			_x setPosATL (_spawnpos vectorAdd _rel);
		} forEach _vehicles;
	};
	case (2): { // air
		private ["_pos","_spawnpos","_height"];
		_pos = [_group,0,2] call FUPS_fnc_randomMarkerPos;
		_spawnpos = if (_pos isEqualTo []) then { getPosATL leader _group } else {_pos};
		_height = (getPosATL vehicle leader _group) select 2;
	...
	case (3): { // ship
		...
		_spawnpos = if (_pos isEqualTo []) then {getPosATL leader _group } else {_spawnpos};
		_spawnpos set [2,0];
